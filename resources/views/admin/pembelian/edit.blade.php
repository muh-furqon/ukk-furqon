...
<div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
    <h2 class="text-2xl font-semibold mb-4">Edit Pembelian</h2>
    ...
    <form action="{{ route('pembelians.update', $pembelian->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="date" name="tgl_beli" value="{{ $pembelian->tgl_beli }}" required class="w-full p-2 border rounded">
        </div>
            ...
            <div class="barang-item flex space-x-2 mb-2">
                <select name="barang_id[]" required class="p-2 border flex-1">
                    ...
                    @foreach ($barangs as $barang)
                        <option value="{{ $barang->id }}" {{ $barang->id == $detail->barang_id ? 'selected' : '' }}>
                            {{ $barang->nama_barang }}
                        </option>
                    @endforeach
                </select>
                <input type="number" name="jumlah_beli[]" required placeholder="Jumlah" class="p-2 border rounded w-20" value="{{ $detail->jumlah_beli }}" oninput="updateTotal()">
                <input type="number" name="sub_total[]" required placeholder="Subtotal" class="p-2 border rounded w-24" value="{{ $detail->sub_total }}" oninput="updateTotal()">
        ...
        <div class="mb-4">
            ...
            <input type="number" name="total" id="total" required class="w-full p-2 border rounded bg-gray-100" readonly value="{{ $pembelian->total }}">
        </div>
...
// Call updateTotal() initially to set the correct total value on load
updateTotal();
</script>
@endsection
